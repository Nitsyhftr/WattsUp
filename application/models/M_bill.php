...
class M_bill extends CI_Model
{
  ...
  public function get_filtered_tagihan($search = null, $status = null)
{
  ...
  if (!empty($search)) {
    ...
    $this->db->or_like('penggunaan.id_penggunaan', $search);
    $this->db->group_end();
  }
  ...
  $this->db->order_by('penggunaan.tahun', 'ASC');
  return $this->db->get()->result();
}


public function get_filtered_tagihan_adm($filters = [])
{
  $this->db->select('tagihan.*, pelanggan.nama_pelanggan, penggunaan.bulan, penggunaan.tahun, tarif.tarif_perkwh');
  $this->db->from('tagihan');
  $this->db->join('penggunaan', 'penggunaan.id_penggunaan = tagihan.id_penggunaan');
  ...
  // Filter berdasarkan ID pelanggan (wajib)
  if (!empty($filters['id_pelanggan'])) {
    $this->db->where('pelanggan.id_pelanggan', $filters['id_pelanggan']);
  }

  // Filter pencarian keyword
  if (!empty($filters['search'])) {
    $this->db->group_start();
    $this->db->like('tagihan.id_tagihan', $filters['search']);
    $this->db->or_like('tagihan.id_penggunaan', $filters['search']);
    $this->db->or_like('pelanggan.nama_pelanggan', $filters['search']);
    $this->db->group_end();
  }

  // Filter status
  if (!empty($filters['status'])) {
    $this->db->where('tagihan.status', $filters['status']);
  }
